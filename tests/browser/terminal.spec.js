...
test('chafa renders symbols and Sixel images through tmux and reconnect', async ({
  page,
  service,
}) => {
  const errors = [];
  page.on('pageerror', (error) => errors.push(error.message));
  await exec('chafa', ['--version']).catch((error) => {
    if (error.code === 'ENOENT')
      test.skip(true, 'chafa is required for the image integration test');
    throw error;
  });
  await login(page, service);
  await page.locator('#new-session').click();
  await page.getByLabel('会话名称', { exact: true }).fill('Image preview');
  await page.getByRole('button', { name: '创建会话', exact: true }).click();
  await expect(page.locator('#connection-status')).toHaveText('已连接');
  const item = (await (await page.request.get(service.url + '/api/sessions')).json())[0];
  const support = await exec('tmux', [
    '-S',
  ...
  test.skip(support.stdout.trim() !== '1', 'tmux must be built with Sixel support');
  const asset = "'" + resolve('web/lantern.svg').replaceAll("'", "'\\''") + "'";
  await run(
    page,
    `chafa --probe off -f symbols --symbols vhalf -s 20x8 ${asset}; printf 'symbols:%s\\n' done`,
  );
  await expect(page.locator('.xterm-rows')).toContainText('symbols:done');
  await expect(page.locator('.xterm-rows')).toContainText(/[▀▄█]/);
  const pixels = (half) =>
    page.locator('canvas.xterm-image-layer').evaluateAll(
      (layers, half) =>
        layers.reduce((count, canvas) => {
          const data = canvas.getContext('2d').getImageData(0, 0, canvas.width, canvas.height).data;
          for (let i = 0; i < data.length; i += 4)
            if (
              (!half || (half === 'left') === (i / 4) % canvas.width < canvas.width / 2) &&
              data[i + 3] > 0 &&
              data[i] > 120 &&
              data[i] > data[i + 1] * 1.2 &&
              data[i] > data[i + 2] * 1.2
  ...
  await run(
    page,
    `clear; chafa --probe off -f sixels --passthrough none -s 20x8 ${asset}; printf 'sixel:%s\\n' done`,
  );
  await expect(page.locator('.xterm-rows')).toContainText('sixel:done');
  await expect.poll(pixels).toBeGreaterThan(100);
  await run(page, "clear; printf 'cleared:%s\\n' ready");
  await expect(page.locator('.xterm-rows')).toContainText('cleared:ready');
  await expect.poll(pixels).toBe(0);
  await run(page, `chafa -s 20x8 ${asset}; printf 'auto:%s\\n' done`);
  await expect(page.locator('.xterm-rows')).toContainText('auto:done');
  await expect.poll(pixels).toBeGreaterThan(100);
  ...
  await run(
    page,
    `chafa --probe off -f sixels --passthrough none -s 12x6 ${asset}; printf 'split:%s\\n' done`,
  );
  await expect(page.locator('.xterm-rows')).toContainText('split:done');
  await expect.poll(() => pixels('right')).toBeGreaterThan(100);
  const select = await page.request.post(`${service.url}/api/sessions/${item.id}/panes/action`, {
    data: { pane: item.active_pane, action: 'select' },
  });
  expect(select.ok()).toBeTruthy();
  await run(
    page,
    `clear; chafa --probe off -f sixels --passthrough none -s 12x6 ${asset}; printf 'left:%s\\n' done`,
  );
  await expect(page.locator('.xterm-rows')).toContainText('left:done');
  await expect.poll(() => pixels('left')).toBeGreaterThan(100);
  await expect.poll(() => pixels('right')).toBeGreaterThan(100);
  await mkdir('.runtime', { recursive: true });
  await page.screenshot({ path: '.runtime/chafa-sixel.png' });
  await page.reload();
  await expect(page.locator('#connection-status')).toHaveText('已连接');
  await expect(page.locator('.xterm-rows')).toContainText('split:done');
  await expect.poll(() => pixels('left')).toBeGreaterThan(100);
  await expect.poll(() => pixels('right')).toBeGreaterThan(100);
  await run(page, "printf 'image-input:%s\\n' intact");
  await expect(page.locator('.xterm-rows')).toContainText('image-input:intact');
  expect(errors).toEqual([]);
});
