...
class CodexStatus:
    ...
    def processes(self, pane: Pane) -> list[psutil.Process] | None:
        ...
        for proc in children:
            try:
                if proc.name() != "codex" or proc.terminal() != pane.tty:
                    continue
        ...
        for pane in panes:
            ...
            if processes is None:
                result[pane.id] = [
                    ...
                    for badge in self.last.get(pane.id, [])
                ]
                continue
    ...
    async def sample(self, panes: list[Pane]) -> dict[str, list[dict]]:
        ...
        now = time.time()
        live = {pane.id for pane in panes}
        ...
        for pane in panes:
            ...
            if badges:
                ...
                self.last[pane.id] = badges
            elif pane.id in self.last:
                ended = self.exited.setdefault(pane.id, now)
                if now - ended < 30:
                    badges = [
                        {**badge, "state": "exited", "source": "process"}
                        for badge in self.last[pane.id]
                    ]
                else:
                    self.last.pop(pane.id)
                    self.exited.pop(pane.id)
            for badge in badges:
                state = badge["state"]
                detail = ""
                if state == "unknown":
